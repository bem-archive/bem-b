...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="xsl:apply-templates | xsl:with-param | xsl:if | xsl:for-each | xsl:variable | xsl:element[contains(@name, '{')] | xsl:value-of | xsl:copy-of | xsl:choose | xsl:when | xsl:otherwise" mode="xsl-inline">
        <xsl:copy>
            <xsl:apply-templates select="." mode="attributes"/>
            <xsl:apply-templates mode="xsl-inline"/>
        </xsl:copy>
